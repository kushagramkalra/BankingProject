...
def withd():
        ...
        if(row>0):
            ...
            var.execute("insert into mini (account_number,date_and_time,withdraw) values('"+ac+"','"+str(datetime.now())+"','"+am+"')")
            conn.commit()
...
frame2=Frame(win,width=600,height=280,highlightbackground="green")
frame2.place(x=20,y=20)
lb1=Label(frame2,text="WITHDRAW CASH",font=('arial',20,'bold'))
...
tb4.place(x=390,y=210)

btn=Button(win,text="WITHDRAW",command=withd,bg="orchid",width=15,height=2,bd=10)
